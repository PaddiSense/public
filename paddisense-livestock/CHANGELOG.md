...
## 2026.8.7 — Log redaction adopted (this addon had NONE) + prod-only Admin keyring
...
## 2026.8.6 — WR-PS-161: paddock-sync Farm URL is operator-configurable

### Changed
- `api/paddocks.py` paddock-sync no longer relies solely on hardcoded Farm addon slugs. A new
  `farm_spatial_url` addon option (env `STR_FARM_SPATIAL_URL`, comma-separated for >1) is tried
  FIRST, then the built-in defaults — so a Farm reinstall (which changes its slug) is fixable per box
  without a rebuild, instead of silently breaking the sync. Default (unset) → unchanged behaviour.
- New `_spatial_urls()` (override-first + de-duped). Test: override-first+dedup / unset→defaults.

## 2026.8.5 — WR-PS-603: licence activation self-heals the livestock_app grant IN-LINE (fresh-box fix)

### Fixed
- On a fresh box the least-priv `livestock_app` role can be missing its DML grants (Core's out-of-band
...
- **F-A2 no-bare-TOFU flag** (`core/module_gate.py::_no_bare_tofu`, env `STR_NO_BARE_TOFU`, default OFF):
...
- New `core/internal_auth.py` (vendored fleet-wide from
  ...
  the shared box master key via HMAC. The paddock proxy (`api/paddocks.py` → Farm/Core `/api/spatial/paddocks`) attaches `internal_headers()` so it keeps working after Farm pinned its ingress. Replaces the subnet trust the
  sibling proxy relied on with cryptographic box-key possession.

## 2026.7.17 — Trust the DEV Admin signing key (dev-box enrolment)

### Changed
- Re-vendored `paddisense_livestock/data/admin_signing_pubkey.json` from canonical `documentation/contracts/admin_signing_pubkey.json` — adds `admin-dev-2026a` beside prod `admin-2026a` so this DEV box verifies DEV-Admin-signed licences (verification is per-key_id, so additive; prod key unchanged). Fleet keyring propagation (Core did the same, v2026.7.58). ⚠ ships the dev key to PROD at the next grower release — per-lane keyring decision owed to Peter+A before a prod cut.

## 2026.7.16 — app-role grant self-heal (WR-PS-201, port of Store v2026.7.9)

### Fixed
- **A missing DB grant no longer surfaces as a bogus "Licence signature verification failed".**
  Livestock's request path runs as the least-priv `livestock_app` role, whose DML grants are
  provisioned out-of-band by Core. On a box where that provisioning never landed (or was lost when
  the DB owner flipped to `livestock_owner` — postgres-created tables aren't covered by an
  `ALTER DEFAULT PRIVILEGES FOR ROLE livestock_owner`), the app role had NO privileges and every
  request-path query 500'd `permission denied`. Reference incident (Store, 2 grower boxes,
  2026-07-30): a licence activate crashed in the save AFTER the signed nonce was consumed, so Core's
  retry was rejected as `invalid_signature` — a misleading signature error that was really a missing
  GRANT.
- Livestock now ensures its OWN grants at startup: `core/db/_pool.py::grant_app_privileges()`
  (`GRANT USAGE ON SCHEMA public` + `SELECT,INSERT,UPDATE,DELETE ON ALL TABLES` +
  `USAGE,SELECT ON ALL SEQUENCES` to `livestock_app`, from the admin pool), called from `main.py`
  startup **between `ensure_database()` and `init_app_pool()`**. Idempotent (`GRANT` is a no-op when
  already held), non-fatal (logs, never raises), self-heals on every restart.
- `api/licence.py::activate_licence` now wraps `_save_licence` via `_save_licence_or_503` → a DB
  failure returns a legible **503** instead of an unhandled 500; the restart that follows also clears
  the burned in-memory nonce so the next push activates cleanly.
- Tests: `tests/test_pool_selfheal.py::test_grant_app_privileges_self_heals_after_revoke` (revoke →
  prove `livestock_app` locked out → grant restores, idempotent) +
  `tests/test_licence_signed.py::TestActivateSaveFailure` (503-not-500). Verbatim port of the
  ASM/Store reference.

## 2026.7.15 — fleet timezone fix (DB session pinned to the box-local zone)

### Fixed
- **Dates/records now follow local time, not UTC — an animal event recorded first thing in the
  morning no longer lands on yesterday.** The shared TimescaleDB session ran in **UTC** while the
  farm/HA is Australia/Sydney (UTC+10/+11), so every `CURRENT_DATE` / `date_trunc(...)` /
  `TIMESTAMPTZ::date` and every `DEFAULT CURRENT_DATE` event-DATE column (weigh/scan/shearing/
  events/breeding) sliced the day on the UTC boundary (~10-11h early = mid-morning local). An event
  captured before ~10-11am local was therefore stored dated **yesterday**.
- Fix in `core/db/_pool.py` (verbatim port of the ASM reference): both DSN builders
  (`_admin_dsn_with` + `_get_app_dsn`) now pin the Postgres **session timezone** to the box's local
  zone via a libpq `options='-c timezone=<zone>'`, sourced by new `_session_tz()` (TZ env →
  `/etc/timezone` → `Australia/Sydney` fallback). All `CURRENT_DATE` / `date_trunc` / `::date` /
  `DEFAULT CURRENT_DATE` now resolve on the LOCAL calendar. TIMESTAMPTZ storage is unchanged (still a
  UTC instant — no data migration); only date interpretation becomes local. **Existing rows are
  unaffected.**
- Tests: `tests/test_session_timezone.py` (session tz is local not UTC; `CURRENT_DATE` == local date).
